...
def extend_weather_feature(df_history: pd.DataFrame, df_weather: pd.DataFrame):
    """
    处理天气特征
    """
...
def main():  
    ...
    #     col_df = time_delay_embedding(df[col], lags=[3, 2, 1, 0, -1], return_Xy=False)
    ...
    # test 2
    df_with_lags, lag_features, target_features = extend_lag_feature_multivariate(df = df, target="load1", exogenous_features=[], lags=[0, 1, 2, 3])
    logger.info(f"df_with_lags: \n{df_with_lags}")
    logger.info(f"df_with_lags.columns: \n{df_with_lags.columns}")
    logger.info(f"lag_features: \n{lag_features}")
    logger.info(f"target_features: \n{target_features}")
    
    # test 3
    df_lags, lag_features = extend_lag_feature_univariate(df=df, target="load1", lags=[0, 1, 2, 3])
